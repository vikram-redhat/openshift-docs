...
This document describes the process to replace an unhealthy etcd member.

The process depends on whether the etcd member is unhealthy because the machine is not running or the node is not ready, or whether it is unhealthy because the etcd pod is crashlooping.
